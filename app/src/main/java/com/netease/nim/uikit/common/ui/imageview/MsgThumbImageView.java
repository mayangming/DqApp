...
public class MsgThumbImageView extends ImageView {
    ...
    public void loadAsResource(final int resId, final int maskId) {
        ...
        GlideUtils.load(getContext().getApplicationContext(), resId, this);
...
//        RequestBuilder builder;
        if (ImageUtil.isGif(ext)) {
            GlideUtils.loadGif(getContext().getApplicationContext(), new File(path), this);
        } else {
            GlideUtils.loadFitCenter(getContext().getApplicationContext(), path, this, width, height);
        }
        //builder.into(this);
    }
