...
//        }
    ...
    public static SpannableString makeSpannableStringTags(Context context, String value, float scale, int align, boolean bTagClickable) {
        ...
        // 处理草稿字样显示红色
        int index = value.indexOf(Constants.DRAFT);
        if (index != -1) {
            mSpannableString.setSpan(new ForegroundColorSpan(Color.RED), index, index + Constants.DRAFT.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        }
