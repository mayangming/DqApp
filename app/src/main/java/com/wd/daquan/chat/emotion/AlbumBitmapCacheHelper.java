...
public class AlbumBitmapCacheHelper {
    ...
    public static void init(Context context) {
        CNLog.d("AlbumBitmapCacheHelper", "init");
    ...
    public void uninit() {
        CNLog.d("AlbumBitmapCacheHelper", "uninit");
        ...
        this.tpe.execute(new Runnable() {
            public void run() {
                if(AlbumBitmapCacheHelper.this.currentShowString.contains(path) && AlbumBitmapCacheHelper.this.cache != null) {
                    Bitmap bitmap = null;
                    if(width != 0 && height != 0) {
                        String hash = DqUtils.md5(path + "_" + width + "_" + height);
                        String tempPath = FileUtils.getInternalCachePath(DqApp.sContext, "image") + "/" + hash + ".temp";
    ...
    private Bitmap getBitmap(String path, int widthLimit, int heightLimit) throws OutOfMemoryError {
        ...
        try {
            ...
            if(widthLimit == 0 && heightLimit == 0) {
                sampleSize = this.computeScale(options, ((WindowManager)((WindowManager)this.mContext.getSystemService(Context.WINDOW_SERVICE))).getDefaultDisplay().getWidth(), ((WindowManager)((WindowManager)this.mContext.getSystemService(Context.WINDOW_SERVICE))).getDefaultDisplay().getHeight());
                Log.d("AlbumBitmapCacheHelper", "sampleSize:" + sampleSize);
            } else {
