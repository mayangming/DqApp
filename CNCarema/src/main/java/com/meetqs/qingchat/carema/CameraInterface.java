...
import com.meetqs.qingchat.carema.util.AngleUtil;
import com.meetqs.qingchat.carema.util.CUtils;
import com.meetqs.qingchat.carema.util.CameraParamUtil;
import com.meetqs.qingchat.carema.util.CheckPermission;
import com.meetqs.qingchat.carema.util.DeviceUtil;
...
public class CameraInterface implements Camera.PreviewCallback {
    ...
    private static final String TAG = "qc_log";
...
//    private String orientation = VideoOrientation.VIDEO_UNKNOWN;
...
//            set.playTogether(animC, animF);
    ...
    public void setZoom(float zoom, int type) {
        ...
        LogUtil.i("mParams.isZoomSupported() = " + mParams.isZoomSupported());
        LogUtil.i("mParams.isSmoothZoomSupported() = " + mParams.isSmoothZoomSupported());
        ...
        switch (type) {
            case TYPE_RECORDER:
                ...
                LogUtil.i("TYPE_RECORDER isRecorder = " + isRecorder);
            ...
            case TYPE_CAPTURE:
                ...
                LogUtil.i("TYPE_CAPTURE isRecorder = " + isRecorder);
...
//
        ...
        if(CUtils.isFastDoubleClick(500))return;
        mCamera.takePicture(null, null, new Camera.PictureCallback() {
            @Override
            public void onPictureTaken(byte[] data, Camera camera) {
                Bitmap bitmap = BitmapFactory.decodeByteArray(data, 0, data.length);
                Matrix matrix = new Matrix();
                if (SELECTED_CAMERA == CAMERA_POST_POSITION) { // 后置摄像头
                    matrix.setRotate(nowAngle);
                } else if (SELECTED_CAMERA == CAMERA_FRONT_POSITION) { // 前置摄像头
    ...
    public void startRecord(Surface surface, float screenProp, ErrorCallback callback) {
        ...
        Log.d("clll", "startRecord:" + nowAngle);
//        //获取第一帧图片
...
//        }
        ...
        rotate = nowAngle;
        Log.w(TAG, "录制屏幕 角度 nowAngle: " + nowAngle);
        ...
        videoFileName = "android_" + mUnique + "_" + System.currentTimeMillis() + ".mp4";
        ...
        if (mediaRecorder != null) {
            ...
            if (isShort) {
                if (FileUtils.deleteFile(videoFileAbsPath)) {
                    callback.recordResult(null, null, 0);
                }
                return;
            }
            doStopPreview();
            Log.w("xxxx", "stop rotate = " + rotate);
            String fileName = saveVideoPath + File.separator + videoFileName;
            callback.recordResult(fileName, videoFirstFrame, rotate);
        }
    }
    ...
    public interface StopRecordCallback {
        void recordResult(String url, Bitmap firstFrame, int rotate);
    }
