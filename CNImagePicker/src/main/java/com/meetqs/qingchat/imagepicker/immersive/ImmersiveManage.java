package com.meetqs.qingchat.imagepicker.immersive;
...
public class ImmersiveManage {
     ...
     */
    public static void immersiveAboveAPI19(Activity baseActivity, int statusBarColor, int navigationBarColor, boolean isDarkStatusBarIcon) {
    ...
    /**
     * 设置状态栏字体颜色
     * @param lightColor true黑色，false白色
     * @param window
     */
    public static void setStatusTextColor(Window window, boolean lightColor) {
        if (lightColor) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
    ...
    public static void setStatusFontColor(Window window, int color) {
        boolean flag = isLightColor(color);
        // 如果亮色，设置状态栏文字为黑色
        setStatusTextColor(window, flag);
    }
}
