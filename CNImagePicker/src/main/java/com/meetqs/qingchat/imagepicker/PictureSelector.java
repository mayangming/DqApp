...
public final class PictureSelector {
    ...
    public static List<LocalMedia> obtainMultipleResult(Intent data) {
        ...
        if (data != null) {
            //List<LocalMedia> localMedias = (List<LocalMedia>) data.getSerializableExtra(PictureConfig.EXTRA_RESULT_SELECTION);
            List<LocalMedia> localMedias =  data.getParcelableArrayListExtra(PictureConfig.EXTRA_RESULT_SELECTION);
            if(null != localMedias) {
                result.addAll(localMedias);
            }
        }
        return result;
    }
    ...
    public static List<LocalMedia> obtainSelectorList(Bundle bundle) {
        ...
        if (bundle != null) {
//            List<LocalMedia> mediaList = (List<LocalMedia>) bundle.getSerializable(PictureConfig.EXTRA_SELECT_LIST);
            List<LocalMedia> mediaList = bundle.getParcelableArrayList(PictureConfig.EXTRA_SELECT_LIST);
            if(null != mediaList) {
                selectionMedias.addAll(mediaList);
            }
        }
    ...
    public static void saveSelectorList(Bundle outState, List<LocalMedia> selectedImages) {
        outState.putParcelableArrayList(PictureConfig.EXTRA_SELECT_LIST, (ArrayList<? extends Parcelable>) selectedImages);
    }
